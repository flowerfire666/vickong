<template>
	<view>
		<view class='collectionGoods' v-if="collectProductList.length">
			<view @click="goDetails(item)" class='item acea-row row-between-wrapper'
			 v-for="(item,index) in collectProductList" :key="index">
				<view class='pictrue'>
					<image :src="item.image"></image>
				</view>
				<view class='text acea-row row-column-between'>
					<view class='name line1' :class="item.is_show == 0?'on':''">{{item.store_name}}</view>
					<view class='acea-row row-between-wrapper'>
						<view class='money font-color' v-if="item.is_show == 1">￥{{item.price}}</view>
						<view class='money on' v-else>该商品已下架</view>
